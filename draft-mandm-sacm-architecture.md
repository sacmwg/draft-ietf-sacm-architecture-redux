...
In {{fig-detailed}}, we have another view of the architecture, but one that fosters the development of a pluggable ecosystem of cooperative tools. ECP is very relevant, but the interface bindings to things like the Repository and Orchestrator are using {{I-D.ietf-mile-xmpp-grid}}. Existing collection mechanisms can be brought into this architecture by specifying the interface of the collector and creating the XMPP-Grid Connector.
